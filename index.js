...
function RateLimiter(options) {
    ...
    if (redis) {
        ...
        return function (id, options, cb) {
            ...
            var clearBefore = now - (interval || options.interval * 1000);

            var batch = redis.multi();
            batch.zremrangebyscore(key, 0, clearBefore);
            batch.zrange(key, 0, -1, "withscores");
            batch.zadd(key, now, uuid());
            batch.expire(key, Math.ceil((interval || options.interval * 1000) / 1000000)); // convert to seconds, as used by redis ttl.
            batch.exec(function (err, resultArr) {
                ...
                var tooManyInInterval = userSet.length >= (maxInInterval || options.maxInInterval);
                var timeSinceLastRequest = now - userSet[userSet.length - 1];

                var result;
                var remaining = (maxInInterval || options.maxInInterval) - userSet.length - 1;

                if (tooManyInInterval || timeSinceLastRequest < (minDifference || options.minDifference)) {
                    result = Math.max(tooManyInInterval ? userSet[userSet.length - (maxInInterval || options.maxInInterval)] - now + interval : 0, (minDifference || options.minDifference) ? (minDifference || options.minDifference) : 0);
